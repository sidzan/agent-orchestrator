...
- **DO NOT run** `pnpm --filter @demo/admin test` directly — only invoke `Skill code-quality`
...
2. Re-run ONLY that specific test file using your project's test runner — pass the test file path as a filter to `pnpm --filter @demo/admin test` (the exact flag depends on the runner: `--reporter=verbose` for vitest, `--verbose` for jest, etc.). Read `references/patterns/testing.md` if it exists for the canonical incantation.
