...
| Setting | Value |
|---------|-------|
| Atlassian site | `https://demo.atlassian.net` |
| Primary project key | `DEMO` |
| Default ticket type | `Task` (engineering work), `Story` (user-facing feature), `Bug` (defect) |
| Default labels | `ai-generated`, `demo-npm` (add to every ticket this skill creates) |
...
> "I'll create a Jira ticket in `DEMO` as a **[Story|Task|Bug]**. Summary: `<short>`. OK to create? (y / change type / change project / skip Jira)"
...
- **Labels:** always attach `ai-generated` and `demo-npm`.
...
3. User unsure → search first with `mcp__claude_ai_Atlassian__searchJiraIssuesUsingJql`:
   ```
   project = DEMO AND summary ~ "<keywords>" ORDER BY created DESC
   ```
