...
1. AskUserQuestion: "Start the `<APP>` dev server (`npm start`), then I'll guide you through saving your auth state."
2. `agent-browser --session-name <APP> open http://localhost:3000`
...
```bash
agent-browser open http://localhost:3000/<feature-path>
agent-browser wait --load networkidle
